...
The three `components/*` modules contain **no ESP-IDF headers** and are compiled
...
- **RGB LED** (WS2812, GPIO8): blue=disconnected, amber=moving, cyan=settling,
  green=idle. *Note:* this board's LED takes **RGB** order while `led_strip`'s
  WS2812 model emits GRB, so `led_set()` swaps R↔G.
- **LCD** (ST7789, 172×320, SPI: MOSI=6 SCLK=7 CS=14 DC=15 RST=21, backlight=22):
  shows state / `angle` / `steps`. Three settings were needed for this panel:
  `swap_bytes = true` (RGB565 byte order — without it text shows a colour fringe),
  `esp_lcd_panel_set_gap(panel, 34, 0)` (the 172-wide panel's RAM offset), and the
  backlight on **GPIO22 via LEDC PWM** at `LCD_BL_DUTY_PCT` (default 50%).
...
Covers `command_parser`, `motion_math`, and `ramp`.
